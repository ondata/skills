...
- `openalex`: nuove sezioni su ricerca per campo, sintassi di `search=` e semantica, tutte misurate contro un articolo bersaglio. La ricerca per titolo perde i lavori che parlano di un paese senza nominarlo nel titolo: `title_and_abstract.search` unito a `authorships.institutions.country_code` dà insiemi piccoli abbastanza da leggersi per intero, che è quello che serve quando il ranking non aiuta. Aggiunto il test di richiamo su un lavoro noto (cercalo per DOI nei risultati e guarda a che posto sta) come passo della validazione: è quello che ha fatto emergere un articolo rimasto al diciannovesimo posto su 468 e mai letto. La sintassi `filter=campo.search:` è deprecata da OpenAlex ma non ha sostituto per lo scoping su un campo, e resta l'unica strada: annotato. Misurato anche il resto: `corpus=all` porta il 2024 da 10,8 a 28,0 milioni di lavori, un filtro `.search` si paga come ricerca (0,001) e non come filtro (0,0001), `meta.cost_usd` dice il costo reale di ogni chiamata, e la semantica rifiuta `from_publication_date` elencando nell'errore i filtri che accetta. Due punti in cui la documentazione non regge alla prova: dà `per_page` massimo 100 mentre l'API accetta 200 e lo dichiara nel proprio messaggio d'errore, e dice che la semantica accetta quasi tutti i filtri mentre la whitelist vera sta nell'errore. Corretta la ricetta 1, che passava `title.search` come parametro a sé (non valido) e senza `api_key`
...
- `cup-cig`: three traps survive the CLI and are documented as such. `righe` defaults to `--limite 50` and returns exactly 50 rows with no warning where the true count is 347, so it must be paired with `conta` or `--tutte --limite 0`; `scarica` passes the dump through as latin-1 (`à` as the single byte `0xE0`) while the OData path is clean UTF-8; `cig` without `--regione` costs 24s against 2s, measured with `--no-cache` — not serial requests, which already run six at a time, but their number: `gar` and `pga` for 21 regions, two calls each
- `cup-cig`: **the decimals trap no longer holds.** The reference claimed a comma in `prg` against a dot in `gar` and `sal`; today it is a dot in every family, on the same `/datastore/dump/` endpoint the reference documents — 866.216 numeric fields with a dot and none with a comma over 66.633 rows of the national `prg`, and the six Sicilian regional files agree. Checked with plain `curl` too, so it is not the CLI normalising anything. A stale `REPLACE(col, ',', '.')` is a no-op, which is the bad kind of wrong: it hides the `CAST` it was protecting. What does vary, and was not documented, is the **quoting**: numeric fields are quoted in the national `prg` and bare in the regional files
- `cup-cig`: `dossier` returns six of the seven MOP families — no `localizzazione` — so it answers everything about a work except where it is. `mop` does map the seventh, `--famiglia localizzazione` works; it is the flag's help text that lists six values, and `--regione` hides the row because Localizzazione is national. How `dossier` picks the region is not what the output suggests: it matches a region name inside `Descrizione Titolare`, so `REGIONE AUTONOMA VALLE D'AOSTA` resolves and `COMUNE DI PALERMO` does not — `regione` comes back `null` and it falls back to querying every partition. It never reads the Localizzazione dataset, so a `null` region says nothing about where the work is
- `cup-cig`: `compatibility` was 520 characters against the 500 the spec allows, and had been for a while. The macOS `coreutils` hint moved into the body next to the other `cig-fetch.sh` requirements, where it is more useful anyway: the field is now 473. No other skill in the repo is over its frontmatter limits
...
- `cup-cig`: the CLI author shipped fixes for everything the report raised, same day, and the reference is realigned to them. Verified on a build of that day's HEAD: `dossier` has a `localizzazione` section with `Codice ISTAT Comune` already concatenated and takes the region from `Codice Regione` instead of guessing it from the owner's name — a CUP of the Comune di Palermo now resolves to Sicilia and the call drops from 34,7s to 9,4s, because the guess used to fail and it queried all 21 partitions; `righe` carries `meta.nota` with the real total when a result touches the limit (`troncato a --limite 50 di 347 righe`) and `results` stays an array; `archivio_vuoto: true` separates an unsynced archive from a genuine miss, and `--home` now moves the archive; `scarica` converts to UTF-8, byte-identical to `iconv -f ISO-8859-1`, with `--raw` for the portal's own bytes; `cig` reads the schema once per family and runs twelve at a time, 18,8s, and 4,1s with `--rate-limit 0`
- `cup-cig`: on the one suggestion turned down, the author is right and the reasoning is worth keeping: an exit code other than zero reads as a failed command, and an unsynced archive is not a failure — the command ran and answered. Hence the boolean field rather than an exit status
- `cup-cig`: the reference now states the date it was checked against, because a build older than that differs on four points and every one of them fails silently. The public library copy was still being published when this was written
...
- `typst-cards`: new `references/design-md.md` — the skill now reads a [DESIGN.md](https://github.com/google-labs-code/design.md) as the source of the theme instead of guessing a palette. Two version numbers not to confuse: the format is at `version: alpha`, the `@google/design.md` tooling has its own release line
- `typst-cards`: the two conversions that bite when a DESIGN.md meets a card. `fontSize` px→pt is ×0.75, but a literal translation makes a screen-sized scale look timid on 1080px — keep the ratios between levels, raise the absolute values. And CSS `lineHeight` is the whole line box while Typst `leading` is the gap added on top, so `(L−1)` collides: subtract ~0.75 (verified, 87px pitch against the 88px prescribed by `lineHeight: 1.1`)
- `typst-cards`: `{token.references}` must be resolved before reaching `theme.typ`, and named CSS colours must be converted to hex — `rgb()` rejects both loudly. `oklch()` is native in typst 0.14.2. Fonts are the quiet failure: a DESIGN.md names brand families that are typically absent, and Typst falls back with a warning, not an error
- `typst-cards`: `components` and elevation largely do not map onto a static card (no hover state, shadows read poorly at 1080px) — stated as a boundary so the mapping stays honest instead of inventing one for text inputs
...
- `cup-cig`: **the `HEAD` → 404 quirk on ItaliaDomani is real and was nearly deleted by mistake.** A first check — one request per file — returned 200 on all four, which looked like proof the documented behaviour was gone. Repeating it five times per file showed the truth: the response is **not deterministic**. `PNRR_Progetti` and `PNRR_Localizzazione` answer 404 most of the time with the occasional 200; `PNRR_Gare` and `PNRR_Soggetti` answer 200 every time. The CDN edge nodes are not configured alike, so which node serves the request decides the answer
- `cup-cig`: the instruction «never probe these URLs with `curl -I`» therefore stays, with the non-determinism now documented — a 404 on `HEAD` proves neither that the file is missing nor that it exists. Added the methodological corollary: a single probe per file is not a measurement here. Confirmed separately that `Range` is genuinely ignored (a ranged request returns `200`, not `206`)
- `cup-cig`: reworded the Akamai block as conditional on the caller's network rather than as a property of the server — from consumer networks a plain `GET` works with no headers at all, from datacenter egress it needs `User-Agent` and `Range` together, and from some egress (GitHub Actions on Azure) nothing works. Stated the consequence: never conclude anything about this source from a single network
