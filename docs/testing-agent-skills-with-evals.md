...
- **Implicit invocation (`test-02`):** This prompt describes _exactly_ the scenario the skill targets, setting up a minimal React + Tailwind demo, without mentioning the skill by name. It tests whether the name and description in `SKILL.md` are strong enough for Codex to select the skill on its own.
...
The value here is that everything is **deterministic and debuggable**. If a check fails, you can open the JSONL file and see exactly what happened. Every command execution appears as an `item.*` event, in order. That makes regressions straightforward to explain and fix.

## 6. Conduct qualitative checks with rubric-based grading
...
- **Command count and thrashing:** Count `command_execution` items in the JSONL trace to catch regressions where the agent starts looping or re-running commands.
...
- **Runtime smoke checks:** Start `npm run dev` and hit the dev server with `curl`, or run a lightweight Playwright check if you already have one.
...
- **Sandbox and permission regressions:** Verify the skill still works without escalating permissions beyond what you intended.
...
- **Use structured grading where rules fall short.** Add a rubric-based pass with `--output-schema` to grade style and conventions reliably.
